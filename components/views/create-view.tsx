...
import { createAnimal, fetchHabitats as fetchHabitsAPI } from "@/lib/utils";
...
export function CreateView() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const dataToSend: any = { ...formData, birthDateTime };
      if (!formData.habitat.id || formData.habitat.id === 0) {
        delete dataToSend.habitat;
      }

      const result = await createAnimal(dataToSend);
      ...
      <Card>
        ...
        <CardContent>
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="space-y-2">
              <Label htmlFor="habitat">Habitat (Opcional)</Label>
              <Select
                value={
                  formData.habitat.id > 0 ? formData.habitat.id.toString() : "0"
                }
                onValueChange={(value) =>
                  setFormData((prev) => ({
                    ...prev,
              ...
              >
                <SelectTrigger className="w-full" id="habitat">
                  <SelectValue
                    className="w-full"
                    placeholder="Selecciona un habitat (opcional)"
                  />
                </SelectTrigger>
                <SelectContent>
